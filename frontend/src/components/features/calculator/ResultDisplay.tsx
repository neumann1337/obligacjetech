...
import { ObliczeniaWynik } from '@/types/calculator';
import { SimpleAreaChart } from '@/components/features/calculator/charts/SimpleAreaChart';
import { SimplePieChart } from '@/components/features/calculator/charts/SimplePieCharts';
...
function WynikGlownyKafelek({ wyniki, formatPLN }: { wyniki: ObliczeniaWynik, formatPLN: (v: number | undefined) => string }) {
      ...
      <div className="w-full">
          ...
          >
              <div className="flex-grow mb-4 md:mb-0">
                  ...
                  <h3 className="text-4xl md:text-5xl font-extrabold tracking-tight">{formatPLN(wyniki.total)}</h3>
              </div>
...
export const ResultDisplay = ({ wyniki, kwota, formatPLN }: { wyniki: ObliczeniaWynik, kwota: number | '', formatPLN: (v: number | undefined) => string }) => {
  return (
      ...
      <div className="flex flex-col gap-8">
          <div className="bg-white rounded-[24px] shadow-[0_10px_40px_rgba(0,0,0,0.04)] border border-black/5 pb-6">
              ...
              <div className="h-[320px] w-full p-4 mt-auto">
                  <SimpleAreaChart data={wyniki.timeline} formatPLN={formatPLN as any} />
              </div>
          </div>
          ...
          <div className="bg-white rounded-[24px] shadow-[0_10px_40px_rgba(0,0,0,0.04)] border border-black/5 p-8 flex flex-col group hover:shadow-lg transition-all duration-500">
              ...
              <div className="flex-grow w-full">
                  <SimplePieChart capital={Number(kwota)} profit={wyniki.total} formatPLN={formatPLN as any} />
              </div>
          </div>
      </div>
    </div>
